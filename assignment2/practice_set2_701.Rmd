...
## Introduction
This assignment focuses on the application of linear regression techniques within a predictive analytics framework. The objective is to estimate model parameters, assess model assumptions, and interpret results using both numerical summaries and visual diagnostics. All analyses are conducted using R to ensure reproducibility.
...
y1=2+3*x1
plot(x1,y1,type='l',col="pink",lwd=3)
...
for (i in 1:5){
  ...
  mod = lm(y ~ x)
  abline(mod, col = i)
  simulation_results$intercept[i] =coef(mod)[1]
  simulation_results$slope[i]= coef(mod)[2]
...
fitting <- lm(y ~ xi_std)
beta_not_hat <- coef(fitting)[1]
beta_hat <- coef(fitting)[2]

beta_not_grid <- seq(beta_not_hat - 2, beta_not_hat + 2, length = 51)
...
for (i in 1:50) {
  for (j in 1:50) {
    RSS[i, j] <- sum((y - beta_not_grid[i] - beta_grid[j] * xi)^2)
  }
}
RSS
...
for(r in 1:R){
  ...
  fit3 <- lm(y3 ~ x3)
  beta_not_hat_vec[r] <- coef(fit3)[1]
  beta_hat_vec[r] <- coef(fit3)[2]
}

predicted_beta_not_hat <- sum(beta_not_hat_vec)/R
predicted_beta_not_hat
...
best_model <- summary_t[which.max(summary_t$R2), ]
best_model
```